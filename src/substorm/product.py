...
def save_list(expansion_phase: pd.DataFrame, recovery_phase: pd.DataFrame, growth_phase: pd.DataFrame, sdir: str,
              sfn: str, stype: str) -> None:
    """
    ...
    for _, exp_row in expansion_phase.iterrows():
        ...
        if recovery_match.empty and growth_match.empty:
            results.append(
                {'growth_phase_start': pd.NaT, 'growth_phase_end': pd.NaT, 'expansion_phase_start': exp_row['start'],
                    'expansion_phase_end': exp_row['end'], 'recovery_phase_start': pd.NaT,
                    'recovery_phase_end': pd.NaT})
        elif recovery_match.empty and not growth_match.empty:
            results.append({'growth_phase_start': growth_match.iloc[0, 0], 'growth_phase_end': growth_match.iloc[0, 1],
                'expansion_phase_start': exp_row['start'], 'expansion_phase_end': exp_row['end'],
                'recovery_phase_start': pd.NaT, 'recovery_phase_end': pd.NaT})
        elif not recovery_match.empty and growth_match.empty:
            results.append(
                {'growth_phase_start': pd.NaT, 'growth_phase_end': pd.NaT, 'expansion_phase_start': exp_row['start'],
                    'expansion_phase_end': exp_row['end'], 'recovery_phase_start': recovery_match.iloc[0, 0],
                    'recovery_phase_end': recovery_match.iloc[0, 1]})
        else:
            results.append({'growth_phase_start': growth_match.iloc[0, 0], 'growth_phase_end': growth_match.iloc[0, 1],
                'expansion_phase_start': exp_row['start'], 'expansion_phase_end': exp_row['end'],
                'recovery_phase_start': recovery_match.iloc[0, 0], 'recovery_phase_end': recovery_match.iloc[0, 1]})
