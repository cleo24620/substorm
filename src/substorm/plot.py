...
def plot(time: pd.Series, imf_bz: pd.Series, ae: pd.Series, al: pd.Series, au: pd.Series, phases: pd.DataFrame,
         if_show: bool = False, if_save: bool = False, sdir: str = './figs') -> Optional[plt.figure]:
    """
    ...
    @return:
    """
    assert len(time) == len(imf_bz) == len(ae) == len(al) == len(au), "Series must all have the same length."
    ...
    assert list(phases.columns) == ['growth_phase_start', 'growth_phase_end', 'expansion_phase_start',
                                    'expansion_phase_end', 'recovery_phase_start',
                                    'recovery_phase_end'], "DataFrame columns do not match the expected list (order-independent)."
    ...
    for _, row in expansion_phase.iterrows():
        if (row['expansion_phase_start'] is pd.NaT) or (row['expansion_phase_end'] is pd.NaT):
            continue
        plt.axvspan(row['expansion_phase_start'], row['expansion_phase_end'], color='red', alpha=0.2)
    for _, row in recovery_phase.iterrows():
        if (row['recovery_phase_start'] is pd.NaT) or (row['recovery_phase_end'] is pd.NaT):
            continue
        plt.axvspan(row['recovery_phase_start'], row['recovery_phase_end'], color='blue', alpha=0.2)
    for _, row in growth_phase.iterrows():
        if (row['growth_phase_end'] is pd.NaT) or (row['growth_phase_end'] is pd.NaT):
            continue
        plt.axvspan(row['growth_phase_start'], row['growth_phase_end'], color='green', alpha=0.2)
